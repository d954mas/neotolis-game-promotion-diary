...
Пока ОБА KEK загружены, ротация обратима:

- Откат «текущей версии»: верни `KEK_CURRENT_VERSION=1` и перезапусти — старые
  строки всё ещё расшифровываются (v1 в `KEK_VERSIONS`).
- Откат самого re-wrap: запусти `pnpm tsx scripts/rotate-kek.ts --to 1` —
  перешифрует DEK обратно под v1 (тот же чекпоинт-механизм, `WHERE kek_version
  < 1` ничего лишнего не возьмёт, кроме строк на v2... используй с
  `--dry-run` сначала).

Откат возможен ТОЛЬКО пока ты не удалил `APP_KEK_BASE64` (v1). После retire (см.
ниже) откат к v1 невозможен — v1 больше не загружен.
...
Должно вернуть `0`. Только тогда retire v1 согласно твоей политике секретов:

1. Promote v2 в слот v1: поставь значение `APP_KEK_V2_BASE64` в `APP_KEK_BASE64`,
   `KEK_CURRENT_VERSION=1`, и убери `APP_KEK_V2_BASE64`.
   ИЛИ — оставь нумерацию как есть (v2 текущий, v1 удалён): просто убери
   `APP_KEK_BASE64`... но v1 — это обязательный слот в `env.ts`
   (`APP_KEK_BASE64` всегда = v1), поэтому канонический retire — это **promote
   v2 в слот v1** (вариант 1).
2. Перезапусти: `docker compose -f docker-compose.prod.yml up -d`.
3. Уничтожь старое значение v1 везде, где оно хранилось (менеджер паролей,
   бэкапы `.env`, тикеты).

После рестарта в `KEK_VERSIONS` останется только новый ключ — утёкший старый KEK
теперь бесполезен.
