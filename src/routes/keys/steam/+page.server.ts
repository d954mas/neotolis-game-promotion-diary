...
/**
 * /keys/steam loader — list the caller's Steam Web API keys.
 *
 * Multi-key: the response is an array of zero or more keys; the UI
 ...
 * through `toApiKeySteamDto` server-side.
 *
 ...
 * deadlock SvelteKit's internal_fetch. `listSteamKeys` enforces the same
 * `userId` scoping the HTTP route does; `toApiKeySteamDto` is the
 * load-bearing ciphertext stripper.
 */
