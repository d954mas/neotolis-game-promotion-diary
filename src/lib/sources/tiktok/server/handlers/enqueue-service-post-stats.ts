...
// adapter_refresh_queue (clone of instagram/server/handlers/enqueue-service-post-
// stats.ts). Advisory-lock per id (serialize concurrent producers) + skip-if-pending
// dedup (the auto path MUST dedup — between enqueue and processing last_polled_at is
...
const ADAPTER_KIND = "tiktok_account";
const QUEUE_NAME = "service_post";

export async function enqueueServiceTikTokPostStats(awemeIds: string[]): Promise<number> {
  const unique = [...new Set(awemeIds.filter((id) => id.length > 0))];
  if (unique.length === 0) return 0;

  return db.transaction(async (tx) => {
    // Deterministic lock order (sorted) avoids deadlocks between concurrent ticks.
    for (const awemeId of [...unique].sort()) {
      await tx.execute(
        sql`SELECT pg_advisory_xact_lock(hashtext(${"tiktok_service_post:" + awemeId}))`,
      );
    }
    ...
    const rows = unique
      .filter((awemeId) => !existingIds.has(awemeId))
      .map((awemeId) => ({
        adapterKind: ADAPTER_KIND,
        queueName: QUEUE_NAME,
        type: "post_stats",
        payload: { post_id: awemeId },
        userId: null,
        priority: 10,
