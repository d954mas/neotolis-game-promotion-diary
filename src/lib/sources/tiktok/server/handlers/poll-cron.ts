// tiktok.poll.cron handler — age-tiered ongoing refresh (clone of instagram/
// server/handlers/poll-cron.ts, single-feed).
//
// Active + Cold + Warm collapse into one queue via boss.schedule({key}); this
// handler reads job.data.tier and dispatches.
//
// What "ongoing refresh" means for TikTok (BACK-03): after backfill_complete, the
// account is NOT walked deeper. Each active/cold tick enqueues a PAGE-1 incremental
// walk (flow='auto_passive') on tiktok.backfill.account — the walker's incremental
// branch fetches page 1 (new posts since the frontier) AND re-polls those posts'
// metrics via writeSnapshot. It does NOT advance the historical cursor. So a single
// page-1 walk is both "new posts since last poll" and "re-poll active posts" in one
// cheap page (D-18: 1 credit).
//
...
import { handleTikTokWarmRefresh } from "./warm-refresh.js";
...
export async function handleTikTokPollCron(job: PollCronJob, boss: MinimalBoss): Promise<void> {
  ...
  if (tier === "warm") {
    await handleTikTokWarmRefresh(job);
    return;
  }
  ...
  if (throttle === "ninetyfive") {
    logger.info(
      ...
      "tiktok.poll.cron: budget at 95% — skipping all background work",
    );
    return;
  }
  if (throttle === "eighty" && tier === "cold") {
    logger.info(
      { jobId: job.id, tier, throttle },
      "tiktok.poll.cron: budget at 80% — skipping cold tier (non-essential)",
    );
    return;
  }
  ...
  // account's newest cached post is within the active window. Cold tier: the newest
  // post is older (or unknown). The newest-post subquery reads tiktok_posts
  // (public-data); the join enforces a live subscriber exists.
  //
  ...
  const newestPostExpr = sql<Date | null>`(
    SELECT MAX(${tiktokPosts.publishedAt})
    FROM ${tiktokPosts}
    WHERE ${tiktokPosts.accountId} = ${dataSourceChannelState.channelKey}
  )`;
  ...
  const candidates = await db
    ...
    .where(sql`${dataSourceChannelState.kind} = 'tiktok_account'`)
    .orderBy(sql`${dataSourceChannelState.lastPolledAt} ASC NULLS FIRST`)
    .limit(MAX_PICK);

  // Partition by tier off the newest cached post. An account with no cached posts
  ...
  if (picked.length === 0) {
    logger.info({ jobId: job.id, tier }, "tiktok.poll.cron: no accounts to poll this tier");
    return;
  }
  ...
  // incremental window; the walker's incremental branch fetches page 1 only.
  ...
  for (const ch of picked) {
    try {
      await boss.send(
        ...
        {
          singletonKey: `tiktok-poll-${tier}-${ch.channelKey}`,
      ...
      logger.warn(
        ...
        "tiktok.poll.cron: enqueue failed for account; continuing",
      );
    }
  }
  ...
  logger.info(
    ...
    "tiktok.poll.cron: tick complete",
  );
}
