// instagram_accounts — our local copy of a public Instagram account's own
// metadata (current @handle / username, full name, avatar, follower count),
// scraped from the data we ALREADY pay for (the profile resolve + the free feed
// owner object).
//
...
// telegram_channels / instagram_posts public-data semantics). ESLint
// TENANT_TABLES allowlist extends `instagramAccounts` with an explicit "public
// external data, no tenant scope" comment so the no-unfiltered-tenant-query rule
// does not trip on legitimate `db.select().from(instagramAccounts)` calls.
//
...
// telegram_channels play (see their "this IS our truth" headers). It is NOT a
// copy of a value another table owns: `data_sources.display_name` is the
// user-facing label a tenant may rename freely; `instagram_accounts.full_name`
// is the upstream account name as scraped from Instagram. The two coexist by
// design and never alias each other (AGENTS.md no-denorm rule forbids caching
// data_sources.display_name HERE, and equally forbids caching this full_name onto
// a data_sources / instagram_posts row). The renameable @handle still lives on
// data_sources.metadata.handle for the provider query; `username` here is the
// current scraped @handle, retained on the entity for the future account page +
// a handle_aliases history (mirrors youtube_channels.handle_aliases /
// telegram_channels.handle_aliases).
//
// PK = `account_id` — the stable IG user id (= instagram_posts.account_id, the
...
// renameable display value.
//
// Populated WITHOUT new credit burns (the cost guardrails / backfill cap are
// untouched):
//   - resolveHandleToAccountId (the create-time PAID profile call already made):
//     UPSERTs the richer profile fields (full_name / avatar / follower_count /
//     username) it reads off the response we already pay for. No extra fetch.
//   - the account walker (the FREE feed response already fetched per page):
...
// instagram_posts.thumbnail_url, #69 P1-A).
//
// Retention forever — NO garbage collection (mirrors youtube_channels /
// telegram_channels / instagram_posts). The row IS the historical anchor for the
// account's per-post stats; future account-level analytics (percentiles /
...
export const instagramAccounts = pgTable(
  "instagram_accounts",
  {
    // Stable IG user id (= instagram_posts.account_id). Rename-proof PK.
    ...
    // Upstream account name (display name as Instagram shows it). OUR truth for
    // the account's own name — NOT a denorm of data_sources.display_name.
    ...
    // youtube_channels.handle_aliases / telegram_channels.handle_aliases).
  ...
  (t) => ({
    ...
    handleAliasesIdx: index("idx_instagram_accounts_handle_aliases").using("gin", t.handleAliases),
  }),
);
