...
//      per (platform, provider), decremented on EVERY spend, and NEVER reset
//      by the cron. This is the absolute "cannot spend what isn't there"
//      ceiling (D-16). The reset cron clears ONLY the daily counter; it MUST
//      NOT touch this balance. Seeded from
//      env.SOCIAL_PROVIDER_PREPAID_BALANCE_CREDITS on first spend (UPSERT).
//
...
// The PREPAID BALANCE — a single running ceiling per (platform, provider).
// NEVER reset by the daily-cap cron (Pitfall 3). Monotonically decremented on
// every spend; reservation returns null once it drops below the units
// requested ("cannot spend what isn't there", D-16).
export const socialProviderBalance = pgTable(
  ...
  (t) => ({
    pk: primaryKey({ columns: [t.platform, t.provider] }),
  }),
);
