...
// adapter_refresh_queue (mirrors instagram enqueue-service-post-stats.ts).
...
//
// NO throttle/budget reserve here (Telegram is free) — the producer just
// enqueues the dedup'd row; the lane worker drains it through the pacer.
...
export async function enqueueServiceTelegramPostStats(postIds: string[]): Promise<number> {
  ...
  return db.transaction(async (tx) => {
    ...
    for (const postId of [...unique].sort()) {
      await tx.execute(
        sql`SELECT pg_advisory_xact_lock(hashtext(${"telegram_service_post:" + postId}))`,
      );
    }

    // eslint-disable-next-line tenant-scope/no-unfiltered-tenant-query -- service_post lane scan: cron writes user_id=NULL across all tenants; telegram_posts is public-data
