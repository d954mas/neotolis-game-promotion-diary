...
//
// The WRITE side (`src/lib/server/audit.ts` writeAudit) is append-only.
// This service lights up the user-visible read: tuple-comparison cursor
// pagination, the action filter (single source of truth: `AUDIT_ACTIONS`
// from src/lib/server/audit/actions.ts), and the DTO projection
// (toAuditEntryDto in dto.ts).
//
// actionFilter is AuditAction[] (empty array = "all" semantics; 1-element
// uses eq; 2+ uses inArray). /audit loader passes
// url.searchParams.getAll("action") directly.
//
// dateRange mirrors /feed (user quote: "В окне аудита нет возможности
// выбрать дату как в feed"). The userId WHERE clause stays FIRST in
// `and(...)` per privacy invariant 1; the dateRange clauses are
// independent of userId and append to filterParts.
//
// Tenant scope: listAuditPage takes `userId: string` first; the SELECT's
// WHERE clause begins with `eq(auditLog.userId, userId)`. The custom
// ESLint rule `tenant-scope/no-unfiltered-tenant-query` fires on any
// query against tenant-owned tables that omits this filter — so the
// absence of warnings on this file is a load-bearing assertion, not
// stylistic preference. Disable comments are NOT allowed.
//
// Mitigation by construction: the userId filter is INDEPENDENT of the
// cursor AND the action filter. Even if an attacker forges a cursor
// encoding another tenant's (created_at, id) coordinates and a
// multi-action filter, the userId WHERE clause filters to the caller's
// rows only. The cross-tenant test in tests/integration/audit.test.ts
// is the runtime assertion.
//
// Cursor format: base64url(JSON.stringify({at: ISO, id})). Tuple
// comparison `(created_at, id) < ($1, $2)` is stable under
// same-millisecond ties: id is UUIDv7 so the (created_at, id) ordering
// is deterministic and strictly decreasing.
...
// Empty array = "all" semantics. Callers pass AuditAction[] directly.
export type AuditActionFilter = AuditAction;

// Optional date-range filter (mirrors /feed). Both ends are inclusive —
// the loader applies the end-of-day shift on `to` so a calendar-day URL
// param yields the same row count as a /feed query for the same day.
 ...
 * property — the userId WHERE clause is. Mitigation is by construction.
 */
 ...
 * field types. The route layer maps this to a clean 422 + Paraglide-keyed
 * error code.
 */
export function decodeCursor(s: string): { at: Date; id: string } {
  try {
    ...
    // `new Date("not-a-date")` returns an Invalid Date sentinel that flows
    // into Drizzle and surfaces as a generic 500 instead of the documented
    // 422 `invalid_cursor`. Reject here so a forged cursor never reaches
    // the query builder.
...
/**
 * Defense-in-depth: validate every entry against AUDIT_ACTIONS (single
 * source of truth). The /audit loader drops invalid entries silently
 * before reaching here, but a forensic / smoke caller could pass a forged
 * value directly. Fail closed.
 */
 ...
 *
 * actionFilter semantics:
 *   - [] (default) — no action filter; lists every action.
 *   - [a] — filters to that single action; uses eq + the (user_id, action,
 *     created_at) composite index.
 ...
 *   holds by construction even with a forged cursor + date filter.
 */
export async function listAuditPage(
  ...
  // Defense-in-depth: validate every entry in the array. The loader
  // drops invalid entries before reaching here, but a forensic / smoke
  // caller could pass a forged value. Fail closed.
  ...
  // (user_id, action, created_at) composite index),
  ...
  // Optional date-range clauses. Independent of userId. Pushed to
  // filterParts so the WHERE structure stays:
