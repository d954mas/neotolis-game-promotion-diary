...
//   - Window:            Pacific calendar day (YouTube quota reset boundary).
 ...
 * above. Adapters that declare `userQuotaCap.windowMinutes` must also expose
 * `observability.slidingWindowQuota`; source-specific counter, error-code,
 * and audit semantics stay on the adapter side of the boundary.
 *
 * On exhaustion:
...
async function _enforceSlidingWindowCap(
  ...
  const slidingWindowQuota = adapter.observability.slidingWindowQuota;
  if (!slidingWindowQuota) {
    throw new Error(`adapter ${adapter.kind} declares windowMinutes without slidingWindowQuota`);
  }

  const result = await slidingWindowQuota.check(dbCtx, userId, flow);
  if (result.allowed) return;

  // Cap exhausted: write the audit row first (forensic trail) then throw.
  // Adapter audit writers may use the non-strict writer so a transient DB
  // hiccup on the audit INSERT doesn't suppress the 429.
  await slidingWindowQuota.writeExhaustedAudit({
    userId,
    ipAddress,
    axis: result.audit.axis,
    cap: result.cap,
    used: result.used,
  });

  throw new AppError(result.message, result.errorCode, 429, {
    cap: result.cap,
    used: result.used,
