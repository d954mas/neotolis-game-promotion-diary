// /api/me service functions.
//
// Tenant scope: every service function takes `userId` first. This is the
// single most important convention in the codebase — it makes
// cross-tenant access impossible-by-construction at the call site,
// because every read must scope by the caller's id explicitly.
//
// `getMe(userId)` is the trivial case: the caller IS the resource. We
// still throw NotFoundError if the row vanished (e.g. account was
// deleted between session issue and this request) — that path becomes
// a 404 at the HTTP boundary, prompting the client to re-auth.
//
// `updateUserTheme`: theme is part of the Better Auth `user` row (see
// schema/auth.ts `themePreference`). Better Auth tables are allowlisted
// by the tenant-scope ESLint rule — the rule does not fire on
// `user`/`session`/`account`/`verification` because those tables are
// owned by the auth lib and always scoped by `userId` from the session
// (the rule's STRUCTURAL match wouldn't be load-bearing). The
// substantive scoping is still here: the WHERE clause is
// `eq(user.id, userId)`, where `userId` comes from the actual session
// (resolved by tenantScope).
...
/**
 * Update the caller's theme preference.
 *
