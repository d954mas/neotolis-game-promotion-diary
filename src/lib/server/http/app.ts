...
// Phase 03.0.1 architecture cleanup — adapters mount their own routes via
// `adapter.registerRoutes(app)`. Replaces the previous direct import of
// `youtubeMetadataRoutes`. Adding Reddit's preview endpoint in Phase 03.1
// means implementing registerRoutes on the Reddit adapter.
...
export function createApp(): Hono<AppContext> {
  ...
  // Phase 03.0.1 architecture cleanup — adapter-driven HTTP routes.
  // Each adapter mounts its own routes via registerRoutes(app). YouTube:
  // POST /api/youtube/fetch-metadata (preview button on /events/new — cache-
  // first, 1 quota unit on miss). Reddit (Phase 03.1) will mount Reddit-
  // specific preview routes via the same hook — no edit here.
