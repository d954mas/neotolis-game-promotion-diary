#!/usr/bin/env bash
# ============================================================
# Unified-events smoke flow
# ============================================================
# Sourced by tests/smoke/self-host.sh. Asserts the register-source ->
# paste -> see-in-feed -> attach -> see-in-games path end to end against
# the production Docker image, plus cross-tenant probes on the
# tenant-owned routes.
#
# Calling convention:
#   unified_events_smoke <base_url> <session_cookie_a> <session_cookie_b>
#
# where:
#   base_url            = "http://localhost:$APP_PORT"
#   session_cookie_a    = HEADER STRING (e.g. "neotolis.session_token=abc...")
#                         already captured from oauth-mock-driver.
#   session_cookie_b    = same shape, user B's cookie.
#
...
# status (must be 404, NEVER 500).
#
# `set -euo pipefail` is inherited from self-host.sh; the function below
# obeys those settings.

# Returns 0 on full pass, exits non-zero (with a descriptive message) on
# any failure. Intentionally uses local variables so it can be invoked
# multiple times during development.
unified_events_smoke() {
  local base="$1"
  local cookieA="$2"
  local cookieB="$3"

  command -v jq >/dev/null 2>&1 || { echo "FAIL: jq required for unified-events smoke flow"; exit 1; }
  [[ -n "$base"     ]] || { echo "FAIL: unified_events_smoke needs base URL";   exit 1; }
  [[ -n "$cookieA"  ]] || { echo "FAIL: unified_events_smoke needs cookieA";    exit 1; }
  [[ -n "$cookieB"  ]] || { echo "FAIL: unified_events_smoke needs cookieB";    exit 1; }

  echo "== register YouTube data_source =="
  ...
  # AGENTS.md invariant 5: DTO MUST NOT carry userId.
  ...
  echo "== create youtube_video event (manual paste path; CI determinism) =="
  # Bypass oEmbed (which would need network egress to youtube.com) and use
  # the free-form POST /api/events with an explicit kind. source_id is
  # therefore NULL on this row, exercising the unified PollingBadge
  # contract on the manual-paste branch.
  ...
  echo "== GET /api/events (the /feed query) returns the new event =="
  local feedResp
  ...
  echo "== create a game so we can attach =="
  ...
  echo "== PATCH /api/events/:id/attach =="
  # The canonical wire shape is {gameIds: string[]}. The legacy
  # {gameId: string | null} alias is still accepted by the route's union
  # schema, but the response DTO ALWAYS uses the new array shape.
  local attachResp
  ...
  echo "== GET /api/games/:gameId/events returns the attached event =="
  ...
  echo "== cross-tenant — userB GET /api/sources/:id must 404 =="
  local statusSrc bodySrc
  bodySrc=$(curl -s -o /tmp/cross-src.txt -w '%{http_code}' \
    -X GET "$base/api/sources/$sourceId" \
    -H "cookie: $cookieB")
  statusSrc="$bodySrc"
  if [[ "$statusSrc" != "404" ]]; then
    echo "FAIL: cross-tenant GET /api/sources/:id status=$statusSrc (expected 404; must NOT be 500)"
    cat /tmp/cross-src.txt 2>/dev/null
    exit 1
  fi
  # AGENTS.md invariant 2 — body must NOT leak forbidden|permission for tenant-owned 404s.
  if grep -Eqi 'forbidden|permission' /tmp/cross-src.txt 2>/dev/null; then
    echo "FAIL: cross-tenant /api/sources body leaked 'forbidden|permission'"
    cat /tmp/cross-src.txt
    exit 1
  fi

  echo "== cross-tenant — userB PATCH /api/events/:id/attach must 404 =="
  # User B creates their own game, then tries to attach userA's eventId to
  # it. Event ownership wins: the service throws NotFoundError on the event
  # lookup before it ever reaches the gameId check, so the response is 404
  # with no leaky body.
  ...
  local statusAttach
  statusAttach=$(curl -s -o /tmp/cross-attach.txt -w '%{http_code}' \
    -X PATCH "$base/api/events/$eventId/attach" \
    -H "cookie: $cookieB" \
    -H "content-type: application/json" \
    -d "{\"gameIds\":[\"$userbGameId\"]}")
  if [[ "$statusAttach" != "404" ]]; then
    echo "FAIL: cross-tenant PATCH /api/events/:id/attach status=$statusAttach (expected 404; must NOT be 500)"
    cat /tmp/cross-attach.txt 2>/dev/null
    exit 1
  fi
  if grep -Eqi 'forbidden|permission' /tmp/cross-attach.txt 2>/dev/null; then
    echo "FAIL: cross-tenant /api/events/:id/attach body leaked 'forbidden|permission'"
    cat /tmp/cross-attach.txt
    exit 1
  fi

  echo "== unified-events smoke flow: PASS =="
}
