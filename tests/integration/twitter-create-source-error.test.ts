// Twitter/X create-source error paths (flipped live by Plan 11-04).
//
// createSource(twitter_account) maps a provider AdapterError to a clean AppError
// (4xx/5xx), NOT an opaque 500, and never leaves a phantom source row:
//   - an unresolvable handle (resolveAccount → null, i.e. missing/suspended/
//     protected — 11-SPIKE.md Q3 null-by-presence) → 422
//     twitter_handle_unresolvable, NO row written (SOC-01, no credit-then-fail).
//   - each AdapterError category maps to its typed code (rate-limited → 429,
//     operator-issue/permanent → 503, transient → 502).
//   - re-adding the same Twitter account → 422 duplicate_source with the
//     Twitter-specific message.
//   - another tenant's Twitter source → 404 (never 403) on read.
//
// canonicalizeOnCreate resolves the pasted handle → the stable Twitter account_id
...
// cross-tenant cases). The provider seam is faked at the twitter tree's
...
const HANDLE_URL = "https://x.com/ConcernedApe";

function okResolve(accountId: string, displayName: string) {
  return {
    result: {
      accountId,
      displayName,
...
async function expectCreateThrows(): Promise<AppErrorType> {
  const user = await seedUserDirectly({
    email: `tw-create-err-${Math.random().toString(36).slice(2)}@t.io`,
  });
  try {
    await createSource(user.id, { kind: "twitter_account", handleUrl: HANDLE_URL }, "127.0.0.1");
...
describe("createSource(twitter_account) — error paths (Plan 11-04)", () => {
  it("[11-04] an unresolvable / suspended / protected handle (resolveAccount → null) → 422 twitter_handle_unresolvable, no phantom source", async () => {
    // 11-SPIKE.md Q3: a missing/suspended/protected handle is HTTP 200 + data:null
    // → resolveAccount returns null → the canonicalize gate maps that to 422
    // twitter_handle_unresolvable (covers all three handle-failure shapes via the
    // SAME null-by-presence signal).
    resolve.next = { result: null };
    const err = await expectCreateThrows();
    expect(err.code).toBe("twitter_handle_unresolvable");
    expect(err.status).toBe(422);
  });

  it("[11-04] rate-limited → 429 twitter_rate_limited (not a 500)", async () => {
    resolve.next = { throwCategory: "rate-limited" };
    const err = await expectCreateThrows();
    expect(err.code).toBe("twitter_rate_limited");
    expect(err.status).toBe(429);
  });

  it("[11-04] operator-issue → 503 twitter_provider_unavailable (not a 500)", async () => {
    resolve.next = { throwCategory: "operator-issue" };
    const err = await expectCreateThrows();
    expect(err.code).toBe("twitter_provider_unavailable");
    expect(err.status).toBe(503);
  });

  it("[11-04] transient → 502 twitter_unreachable (not a 500)", async () => {
    resolve.next = { throwCategory: "transient" };
    const err = await expectCreateThrows();
    expect(err.code).toBe("twitter_unreachable");
    expect(err.status).toBe(502);
  });

  it("[11-04] re-adding the same Twitter account returns 422 duplicate_source", async () => {
    resolve.next = okResolve("44196397", "Stardew Valley");
    const user = await seedUserDirectly({
      email: `tw-dup-${Math.random().toString(36).slice(2)}@t.io`,
    });
    // First add resolves + persists (autoImport:false → no backfill enqueue).
    const first = await createSource(
      user.id,
      { kind: "twitter_account", handleUrl: HANDLE_URL, autoImport: false },
      "127.0.0.1",
    );
    expect(first.kind).toBe("twitter_account");
    expect(first.channelId).toBe("44196397");
    ...
    try {
      ...
      expect(err.status).toBe(422);
      expect(err.message).toMatch(/Twitter account/i);
    }
  });

  it("[11-04] another tenant's Twitter source returns 404 (never 403)", async () => {
    resolve.next = okResolve("44196397", "Stardew Valley");
    const owner = await seedUserDirectly({
      email: `tw-owner-${Math.random().toString(36).slice(2)}@t.io`,
    });
    const intruder = await seedUserDirectly({
      email: `tw-intruder-${Math.random().toString(36).slice(2)}@t.io`,
    });
