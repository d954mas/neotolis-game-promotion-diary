...
describe("FeedCard anySelected sweep (Wave 2 Plan 08)", () => {
  it.skip(
    "Wave 2 Plan 08: when anySelected=false, card-surface click calls onOpenDetail(id)",
    async () => {},
  );
  it.skip(
    "Wave 2 Plan 08: when anySelected=true, card-surface click calls onToggleSelect(id, !selected) instead of onOpenDetail (D-12 Gmail sweep)",
    async () => {},
  );
  it.skip(
    "Wave 2 Plan 08: click on .card-select checkbox does NOT bubble to card surface — closest('.card-select') early-return",
    async () => {},
  );
  it.skip(
    "Wave 2 Plan 08: click on .card-actions ⋯ button does NOT bubble to card surface",
    async () => {},
  );
  it.skip(
    "Wave 2 Plan 08: data-selected='1' attribute applied when selected=true (LB-10)",
    async () => {},
  );
  it.skip(
    "Wave 2 Plan 08: data-view attribute reflects view='feed' | 'trash' prop",
    async () => {},
  );
});
