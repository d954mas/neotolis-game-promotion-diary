...
declare module "vitest/internal/browser" {
  interface BrowserCommands {
    ...
    // Expose getComputedStyle(document.body).overflowX so a public-routed
    // test can assert body's overflow-x is `clip` (not `hidden`) — the
    // load-bearing distinction that keeps `position: sticky` working on
    // AppHeader.
