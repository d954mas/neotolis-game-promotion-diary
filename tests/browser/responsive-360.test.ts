 ...
 *
 * 360px viewport contract: every page renders without horizontal scroll at
 * 360px viewport AND the primary CTA above the fold is reachable without
 * zoom. The mobile-360px contract is the load-bearing first-screen contract
 * for the indie self-host operator opening the dashboard on a phone.
 *
 * Scope: PUBLIC routes ('/', '/login') only. Authenticated-route 360px
 * sweep (/feed, /sources, /games, /games/[id], /events, /audit,
 * /keys/steam, /settings) is deferred to manual checkpoint — the
...
// `it.skip` placeholders so the contract surface stays visible to a
// future task that lifts the cookie-injection harness; also so a test
// runner enumeration sees every route by name even when the assertion
// is deferred.
const AUTH_ROUTES = [
  "/feed",
  "/sources",
...
describe("sticky + body-scroll-lock at 360px", () => {
  it.skip("AppHeader stays at viewport top after scrolling 200px (deferred — auth harness)", async () => {
    ...
    //
    // Asserts: position: sticky; top: 0 on .header anchored against the
    // layout-root flex column.
  });
  ...
  it.skip("FiltersSheet locks body scroll on open + restores on close (deferred — auth harness)", async () => {
    ...
    //
    // $effect sets overflow:hidden on showModal() and the cleanup function
    // (+ onDialogCancel) restores it. Covers /feed and /audit reuse.
  });
});

/**
 * Sticky AppHeader regression fix at 360px.
 *
 * Diagnosis: position: sticky; top: 0 on .header in AppHeader.svelte plus
 * a layout-root flex column with min-height 100vh in +layout.svelte. The
 * sticky scaffold stopped engaging because src/app.css set
 * `overflow-x: hidden` on html+body. Per CSS spec, when `overflow-x: hidden`
 * is paired with `overflow-y: visible`, browsers coerce `overflow-y` to
 * `auto`, which establishes a scroll container on <body> — and
 * `position: sticky` on a descendant anchors to the FIRST scrolling
 * ancestor. With body promoted to a scroll container, the .header
 * anchored against body (not the viewport), so it scrolled away with
 * content.
 *
 * Fix: src/app.css changed from `overflow-x: hidden` to `overflow-x: clip`.
 * Per CSS spec, `clip` crops overflow WITHOUT establishing a scroll
 * container; `position: sticky` descendants once again anchor to the
 * viewport. Supported in Chrome 90+, Firefox 81+, Safari 16+ — well
 * within the project's modern-browser baseline.
 *
 ...
 * assertion still requires the cookie-injection auth harness, so it lives
...
/**
 * /audit FiltersSheet has no date axis.
 *
 * 'date' was previously a sheet axis label on /audit AND there was a
 * page-level <DateRangeControl> above <FilterChips>. Round-4 UAT surfaced
 * the duplication — the user sees the same date controls in two places.
 * We removed 'date' from /audit's schema axes; <DateRangeControl> stays
 * as the single source of truth for date filtering on /audit. /feed
 * FiltersSheet schema is unchanged (no duplication on /feed —
 * DateRangeControl is the always-visible primary entry, the in-sheet date
 * axis is the secondary entry that the user actively wants on /feed).
 *
 * Component-level regression guard: tests/integration/audit-render.test.ts
 * covers the SSR-level assertion that /audit FiltersSheet renders only
 * the action axis. The browser-mode end-to-end (open sheet at 360px →
 * only audit_action axis visible) requires the auth harness.
 */
describe("/audit FiltersSheet has no date axis", () => {
  it.skip(
    "/audit FiltersSheet renders ONLY action axis (no date / source / kind / show / authorIsMe leak) (manual UAT — auth harness deferred)",
  );
});

describe("PageHeader inline + SourceRow Mine + /games/[id] two-card at 360px", () => {
  it.skip(
 ...
 *   - Remove button visible only in edit mode (read-mode .actions
 *     hosts ONLY the Edit pencil; Remove moves to edit-form footer).
 *   - Edit pencil hidden inside edit mode (no duplicate edit affordance
 *     once the user is already editing).
 *   - auto_import single source of truth (one checkbox bound to
 *     editAutoImport; no parallel <input type="text"> control).
 *   - Save / Cancel / Remove sit at the BOTTOM of the edit-form
 *     block, separated from the form fields by a section divider. User
 *     quote: "Кнопки save cancel нужно внизу карточки делать, иначе
 *     не очевидно где они и зачем".
 *
 ...
 * does not have access to. The component-level regression guard lives in
 * tests/integration/audit-render.test.ts — SSR-level grep + structural
 * assertions on SourceRow.svelte source — and is the load-bearing
 * protection against drift.
 *
 * The four browser-mode tests below are stub-skipped placeholders so a
 * future task that lifts the auth harness sees the contract surface by
 * name and flips it to a live assertion. Each placeholder carries the
 * manual-UAT recipe in its skip-reason so the human reviewer can
 * reproduce the assertion at the keyboard.
 */
describe("SourceRow edit-mode polish at 360px", () => {
  it.skip(
    "/sources SourceRow read-mode shows ONLY Edit pencil (no Remove icon) (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/sources SourceRow edit-mode hides the read-mode Edit pencil + reveals Remove inside form-footer (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/sources SourceRow edit-mode renders auto_import as EXACTLY one input[type=checkbox] (no parallel <input type=text>) (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/sources SourceRow edit-mode form-footer (Save/Cancel/Remove) sits BELOW the form fields — getBoundingClientRect().top of footer > rect.bottom of last form field (manual UAT — auth harness deferred)",
  );
});

/**
 * /games/[id] redesign + StoresSection + edit-mode Remove + Mine accent
 * token unification at 360px.
 *
 * Covers: Mine token unification across FeedCard + SourceRow; oversized
 * 2-card layout retired in favor of lean game header + StoresSection +
 * vertical FeedCard list; Stores section header 'Магазины / Stores' +
 * + Add CTA + collapsible AddSteamListingForm; Steam listing duplicate
 * inline error with existingGameId deep link; SteamListingRow
 ...
 * the cookie-injection auth harness.
 *
 * Stub-skipped here for grep discoverability when the harness arrives;
 * manual UAT covers the visual + click flow. The component-level
 * regression guards live in tests/integration/audit-render.test.ts.
 */
describe("/games/[id] redesign + StoresSection + edit-mode Remove + Mine token at 360px", () => {
  it.skip(
    "/games/[id] no longer renders <section class='game-header-card'> + <section class='events-feed-card'> oversized 2-card layout (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/games/[id] renders <section class='game-header'> + <StoresSection> + <section class='events-feed'> three-section layout (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/games/[id] StoresSection header text === m.stores_section_heading() and renders + Add CTA (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/games/[id] page-level Edit toggle at editMode=false → SteamListingRow.remove-btn is NOT in the DOM (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/games/[id] page-level Edit toggle at editMode=true → SteamListingRow.remove-btn IS in the DOM with aria-label === m.steam_listing_remove_aria() (manual UAT — auth harness deferred)",
  );
  it.skip(
  ...
  it.skip(
    "/games/[id] AddSteamListingForm POST returning 422 with body.error==='steam_listing_duplicate' renders inline .duplicate-error <p> with <a href=`/games/${existingGameId}`> (manual UAT — auth harness deferred)",
  );
  it.skip(
    "AddSteamListingForm 422 with body.metadata.existingState==='soft_deleted' renders the soft-deleted explanation copy (no link) (manual UAT — auth harness deferred)",
  );
  it.skip(
    "Mine token consistency: getComputedStyle(FeedCard.mine).borderLeftColor === getComputedStyle(SourceRow.mine).borderLeftColor (--color-mine resolves to same value on both surfaces) (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/games/[id] events feed renders no FilterChips / no DateRangeControl / no FiltersSheet (pure list per user direction) (manual UAT — auth harness deferred)",
  );
});

/**
 * /events/[id]/edit multi-select Game picker at 360px.
 *
 * The single-select picker ("могу выбрать только одну игру") is replaced
 * with a checkbox-list bound to `gameIds: string[]`. Submit sends
 * `{gameIds: [...]}` unchanged through the existing PATCH
 * /api/events/:id/attach endpoint (the backend already accepts the array).
 * User can attach an event to ≥2 games from the UI.
 *
 * The full end-to-end at 360px (load /events/[id]/edit, see 3 checkboxes,
 * click 2, toggle standalone, see inline conflict + disabled Save) requires
 * the cookie-injection auth harness.
 *
 * Stub-skipped here for grep discoverability when the harness arrives. The
 * component-level regression guard lives in
 * tests/integration/audit-render.test.ts + the round-trip service-layer
 * test in tests/integration/events-attach.test.ts.
 *
 ...
 */
describe("/events/[id]/edit multi-select Game picker at 360px", () => {
  it.skip(
    "/events/[id]/edit at 360px renders <fieldset class='game-picker'> + 3 input[type=checkbox] (NOT a <select> for game picker) (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/events/[id]/edit at 360px clicking 2 checkboxes builds gameIds=[a,b] in component state (each clicked checkbox.checked === true) (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/events/[id]/edit at 360px standalone toggle ON + any checkbox checked → .conflict-error <p> visible AND .submit button has [disabled] attribute (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/events/[id]/edit at 360px game-list scrolls vertically (no horizontal overflow) when user owns 6+ games (manual UAT — auth harness deferred)",
  );
});

/**
 * UI bundle covering six findings on the same UI surface:
 *
 *   - /games/[id] three-section restructure (Игра / Магазины / Лента);
 *     section-header-collocated CTAs; StoresSection grid layout;
 *     SteamListingRow card with Steam deep-link; FeedCard 3-per-row grid
 *     on /games/[id] only.
 *   - FeedQuickNav sticky below AppHeader + PageHeader.
 *   - :root:has(dialog[open]) blocks mouse-wheel scroll.
 *   - FiltersSheet source list shows kind glyph + label.
 *   - PageHeader sticky on /feed, /games, /audit (was /sources only).
 *   - Recently-deleted CTA in PageHeader → modal dialog (the anchor link
 *     approach broke on infinite-scroll surfaces).
 *
 ...
 * tests/integration/audit-render.test.ts — asserts the three-section
 * structure (game-info / stores / events with id="section-game" /
 * "section-stores" / "section-events"); the shared-PageHeader sweep
 * includes /audit. Stub-skipped placeholders here keep the end-to-end
 * contract surface visible to the future task that lifts the auth
 * harness; manual UAT (Russian) covers the visible flow.
 *
 * Manual UAT recipe:
 ...
 *      automatically when the last item is restored.
 */
describe("UI bundle", () => {
  it.skip(
    "/games/[id] renders three labelled sections (#section-game + #section-stores + #section-events) with .section-header rows (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/games/[id] StoresSection grid: single column at 360px viewport, 3-per-row at >=900px (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/games/[id] FeedCard grid: .feedcard-grid resolves to >= 3 columns at 1024px viewport, single column at 360px (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/games/[id] SteamListingRow renders as <article class='store-card'> with Steam icon + listing.name + listing.label + listing.releaseDate + 'Open in Steam' deep-link CTA → store.steampowered.com/app/{appId} (manual UAT — auth harness deferred)",
  );
  it.skip(
    ":root:has(dialog[open]) overflow-hidden blocks mouse-wheel scroll over backdrop — needs evaluateScript browser command (deferred with the auth harness; CSS rule asserted via grep at the integration layer + manual UAT)",
  );
  it.skip(
    "/feed PageHeader stays pinned to top: var(--app-header-height) after window.scrollTo(0, 200) (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/games PageHeader stays pinned to top: var(--app-header-height) after scroll (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/audit PageHeader stays pinned to top: var(--app-header-height) after scroll (manual UAT — auth harness deferred)",
  );
  it.skip(
    "/feed PageHeader renders <button class='recovery-link'> when data.deletedEvents.length > 0; clicking opens <RecoveryDialog> modal (Escape / backdrop / × close; restore returns to original scroll position) (manual UAT — auth harness deferred)",
  );
});
