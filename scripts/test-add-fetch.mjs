...
p.on("response", (r) => { if (r.url().includes("/preview-url")) console.log(`[api] ${r.status()}`); });
...
const fetchedSrc = await dialog.locator(".field-fetch-result").textContent().catch(() => "(no fetch-result)");
console.log("[title]", JSON.stringify(titleVal));
console.log("[kind active]", kindActive?.trim());
console.log("[fetch-result]", fetchedSrc?.trim());

const urlError = await dialog.locator(".field-error").textContent().catch(() => null);
