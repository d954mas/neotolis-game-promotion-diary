...
- **API quotas — per-platform model (revised 2026-05-05 during Phase 3.0 discuss)**: YouTube Data API quota is per Google Cloud project (10k units/day, pay-as-you-go above; data is public, snapshots shareable across tenants by `video_id`) → **service-level model in Phase 3.0**: operator's `SERVICE_YOUTUBE_API_KEYS` env list (round-robin), shared `youtube_video_snapshots` cache, scheduler throttles auto-import + cold polling at 80% quota, admin allowlist sees state via `/admin/quota`; per-user override deferred to Phase 6 trigger. Reddit API is per-user OAuth app (Pitfall 11 — Reddit bans shared OAuth apps for bot-like behavior) → **per-user model in Phase 3.1** (3 ciphertext column-sets per row). Steam Web API is per-publisher (one key per Steamworks account, grants wishlist + sales for that publisher only) → **per-user model in Phase 3.2** (key already shipped Phase 2). Original blanket "per-user keys distributes quotas" rule did not survive contact with YouTube's public-data + UX-friction reality.
...
| ~~Per-user API keys (YouTube/Reddit/Steam), encrypted at rest~~ — split per-platform 2026-05-05 (Phase 3.0 discuss) | Original blanket "per-user keys" rule applied uniformly across platforms; Phase 3.0 discuss surfaced that YouTube is meaningfully different (public data, snapshot-shareable, Cloud-Console friction for typical indie devs). Reddit / Steam keep per-user; YouTube goes service-level. | — Superseded |
| Service-level YouTube API key (operator's, env list `SERVICE_YOUTUBE_API_KEYS`) | Public data → snapshot-shareable across tenants by `video_id`; eliminates per-user Cloud-Console friction; one envelope at 10k unit/day default with round-robin across N keys (tip: 2–3 keys max under one Google account, otherwise paid tier safer). Self-host operator sets own service key in env. | — Pending Phase 3.0 |
| Per-user Reddit OAuth credentials (BYO Reddit app: client_id + client_secret + refresh_token, envelope-encrypted at rest) | Reddit API mandates per-user-identity polling — Pitfall 11 (Reddit bans OAuth apps for bot-like behavior under one shared identity). Three ciphertext column-sets per row (Phase 2 D-08). | — Pending Phase 3.1 |
| Per-user Steam Web API key, envelope-encrypted at rest | Per-publisher data (one key per Steamworks account, grants wishlist + sales for that publisher only — cannot share across tenants). | — Phase 2 (key UI shipped); Phase 3.2 lands wishlist polling |
...
- **`api_keys_*`** — per-tenant encrypted credentials (envelope encryption: KEK from env, DEK per row). Steam in Phase 2 (per-publisher data). Reddit OAuth (3 ciphertext columns per row: client_id / client_secret / refresh_token) in Phase 3.1. YouTube — **NO per-tenant table in Phase 3.0**: service-level operator key from env (`SERVICE_YOUTUBE_API_KEYS`); per-user `api_keys_youtube` override deferred to Phase 6 (trigger: ≥1 power user trips 95% of operator's daily quota).
