...
- **API quotas — per-platform model (revised 2026-05-05 during Phase 3.0 discuss)**:
  - **YouTube**: 10,000 units/day per operator key; service-level model multiplies envelope by `SERVICE_YOUTUBE_API_KEYS.length` (typically 1–3 keys per Google ToS soft-limit). 80% / 95% throttle gates pause Cold + auto-import / Active polling respectively. Data is public; snapshots shareable across tenants by `video_id` via shared `youtube_video_snapshots` cache. Admin allowlist sees state via `/admin/quota`. Per-user override deferred to Phase 6 trigger (≥1 power user trips 95% of operator's quota).
  - **Reddit**: 60 requests/minute per OAuth app; per-user keys distribute the rate-limit envelope per tenant (Pitfall 11 — Reddit bans shared OAuth apps for bot-like behavior). 3 ciphertext column-sets per row — Phase 3.1 model.
  - **Steam**: 100,000 calls/day per Web API key; per-user keys distribute by tenant (per-publisher data — one key per Steamworks account, grants wishlist + sales for that publisher only).

  Original blanket "per-user keys distributes quotas" rule did not survive contact with YouTube's public-data + UX-friction reality.
...
| YouTube — operator-owned service-level keys (`SERVICE_YOUTUBE_API_KEYS` env, round-robin across keys, shared `youtube_video_snapshots` cache by video_id) | Public-data semantics: same view count regardless of who polled. Per-user keys add Cloud Console UX friction for indie devs. Per-user override deferred to Phase 6 trigger (≥1 power user trips 95% of operator's quota). | — Pending Phase 3.0 |
| Reddit — per-user OAuth (BYO Reddit app credentials, 3-ciphertext-column row in `api_keys_reddit`) | Reddit ToS pins API access to a specific user identity (Pitfall 11 shadowban risk). Per-user keys are the only viable model. | — Pending Phase 3.1 |
| Steam — per-user per-publisher API keys (envelope-encrypted in `api_keys_steam`) | Steam Web API key is tied to a Steamworks publisher account; the user's own data lives behind their own key. | — Phase 2 (key UI shipped); Phase 3.2 lands wishlist polling |
...
*Updated 2026-05-05 per Phase 3.0 CONTEXT DV-1.*
...
---
*Last updated: 2026-05-11 after Phase 03.0.2 (Dependency Refresh) — 10 atomic major-bump commits on `feat/phase-03.0.2-dependency-refresh` (PR #26): `@types/supertest` 6→7, `oauth2-mock-server` 7→8, `dotenv` 16→17 (silenced advertising line), ESLint core family 9→10 (+ `@eslint/js` 10, `eslint-config-prettier` 10, `globals` 17), `eslint-plugin-svelte` 2→3 + `svelte-eslint-parser` 0.43→1.6, `pino` 9→10 + `pino-pretty` 11→13 (D-07 redact tripwire green), `@hono/node-server` 1→2, `typescript` 5.6→6.0 (zero source fix-ups), drizzle pair refresh (Path A: drizzle-orm re-pin + drizzle-kit caret bump; `pnpm db:check` no-diff), `better-auth` 1.6.9→1.6.10 (D-07 OAuth smoke green). Every commit landed CI-green on all four jobs (`lint-typecheck`, `unit-integration`, `browser-tests`, `smoke`). Behavioral identity preserved — only 5 source/test files edited (10 lines total) for ESLint-10-induced fixes, all semantically equivalent to pre-fix code. D-07 invariants intact: REDACT_PATHS unchanged, `env.ts` sole `process.env` reader, `src/lib/auth.ts` byte-identical, drizzle migrations untouched (D-08). 2026-05-08 after Phase 03.0.1 (Source Plugin Architecture) — restructured 03.0 YouTube polling pipeline into per-source plugin folders. All YouTube-prefixed code lives under `src/lib/sources/youtube/{server,ui}/` with co-located schema/http/quota/snapshots/handlers/route/UI; cross-source plumbing (events, data-sources, ingest, refresh-poll, tier-resolver, poll-eligibility, audit, purge-account) STAYED at `src/lib/server/`. SourceRegistry dispatches via `getAdapter` / `getAdapterUI`; cross-source code never switches on `source.kind`. Widened `DataSourceAdapter` contract carries `parseUrl` + `observability` + `registerQueues` + `scheduleCronTicks` + `backfillSource` + `canRefreshPoll` per adapter. AdapterError 5-category taxonomy (`transient` / `rate-limited` / `not-found` / `permanent` / `operator-issue`) wired into chargedFetch. Per-kind queue topology: `youtube.poll.cron` (Active+Cold via pg-boss v11+ key-based schedules), `youtube.poll.user`, `youtube.backfill.user`, `youtube.rehab`, `youtube.quota_reset`, `youtube.channel_context_backfill` — old kind-agnostic `poll.active`/`poll.cold`/`poll.user`/`scheduler.tick.*` queues retired via forward-only migration 0021. Generic `POST /api/sources/:id/refresh-content` endpoint + Pull-new-content button on `/sources/[id]`. SOURCE-REFERENCE.md sections 1-7 lands at repo root as the contributor guide. Behavioral identity preserved (POLL-01..06, INGEST-02..04, EVENTS-01..03, SOURCES-01..02, FEED-01 unchanged). 2026-05-06 after Phase 3.0 post-build refactor — moved polling state from per-event copies on `events` to per-video columns on `youtube_videos`; tier classification now keyed on video age (publishedAt), not event age (occurred_at); scheduler dedups by external_id (one HTTP per video per tier-tick); new `'pending'` tier for the brief channel-context-backfill window; weekly rehab cron with poll_failure_count cap recovers privacy-unflipped videos. Earlier: 2026-05-05 after /gsd:discuss-phase 03 — split monolithic Phase 3 into Phase 3.0 / 3.1 / 3.2 (DECIMAL SPLIT); revised API-quota Key Decision to per-platform (YouTube service-level, Reddit/Steam per-user). 2026-05-04 after Phase 02.2 (ship-to-prod) — production deploy artifacts + GDPR baseline + per-user quotas + 9 validated requirements moved Active → Validated.*
