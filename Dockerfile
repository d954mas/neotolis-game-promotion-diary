...
#
# One image, three roles. The same artifact boots as `app`, `worker`, or
# `scheduler` — selected at runtime by the APP_ROLE env var. SvelteKit's
# node-adapter produces build/server.js which is the single Node
# entrypoint; the role-dispatch lives inside that entrypoint.
#
# Multi-stage:
...
#
# Base image: node:22-alpine. Distroless was considered and rejected
# because the alpine base is small enough (~150–200 MB final image) and
# gives us `wget` for HEALTHCHECK without a separate curl install.
# Revisit if a future feature wants sharp or another musl-fragile native
# lib.
...
# OCI image labels. These tie the GHCR image to the source repo so a
# self-host operator can `docker inspect` and trace back to the code.
# No runtime effect.
LABEL org.opencontainers.image.source="https://github.com/d954mas/neotolis-diary"
LABEL org.opencontainers.image.licenses="MIT"
WORKDIR /app
# Non-root user. UID 10001 is well above the alpine reserved range (<1000) and the
...
USER app
EXPOSE 3000
# Healthcheck binds to /readyz. Cloudflare Tunnel polls /healthz separately to avoid
...
# APP_ROLE dispatch lives inside build/server.js.
